...
## 16. Migration status (as of 2026-07-11 — keep this section current as the gap closes)

This file now documents the target dual-assessment architecture (§13/§14). The codebase does not implement it yet. Known gaps, so nobody — human or Claude Code — mistakes the doc for the current state:

- **No independent likelihood/confidence on the AI side.** `AiResultsSummary` (`packages/shared-types/src/aiResultsSummary.ts`) has `overview`, `strengths`, `areasToWatch`, `notedByCaregiver`, `generatedAt` — no `likelihood`, no `confidence`, no `reasoning`/`developmentalProfile`/`supportPriorities`/`homeRecommendations`/`schoolRecommendations`/`professionalPriorities` fields required by §13. `ai-summary-schema.ts`'s Zod schema needs the matching additions, and the prompt behind `claude-ai-summary.client.ts` needs rewriting to request them.
- **No comparison engine.** There is no code that compares Assessment A's and Assessment B's output, computes agreement/disagreement, or attributes a disagreement reason (§13).
- **No Comparison Section in the UI.** `ResultsScreen.tsx` renders Assessment A's results plus the existing AI narrative as supplementary text, not as a clearly separated Section A / Section B / Comparison layout (§14).
- **Missing components:** `<AIAssessmentCard />`, `<ComparisonCard />`, `<ConfidenceBadge />`, `<SupportPrioritiesCard />` (§6) don't exist yet.
- **What's already correct and doesn't need to change:** Assessment A's exclusive ownership of its own fields (rule 7, §2); the AI narrative already never sees Assessment A's computed score (`aiResultsSummary.ts` comment, matches rule 7); strengths-first ordering; the disclaimer, banned-words lint, and red-flag independence all already hold and extend cleanly to Assessment B under this update.
- Do not start building §13/§14/§6's "not yet built" items without a plan reviewed against rule 16 (§2) — this is an architecture change requiring the same sign-off discipline as clinical content (§9).
